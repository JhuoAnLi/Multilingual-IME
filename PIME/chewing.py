...
from .chewing_config import (
    chewingConfig,
    SWITCH_LANG_WITH_BOTH_SHIFT,
    SWITCH_LANG_WITH_LEFT_SHIFT,
    SWITCH_LANG_WITH_RIGHT_SHIFT,
)
import sqlite3
...
# 建立一個文件處理器，並指定日誌儲存的位置
file_handler = logging.FileHandler(r"C:\Users\super\Desktop\test.log")
...
class ChewingTextService(TextService):
    def __init__(self, client):
        ...
        self.temp_inputstring = ""
        self.int_selkey = 0
        self.candidateword = ""
        self.candidatelength = 0
        self.candidates = []
        self.dyna_best_result = ""
        self.int_CursorInStr = 0
        self.cursor_position = 0
        self.check_commit = False
    ...
    def initChewingContext(self):
        ...
        if not self.chewingContext:
            ...
            search_paths = ";".join((cfg.getConfigDir(), CHEWING_DATA_DIR)).encode(
                "UTF-8"
            )
            ...
            if self.lastShapeMode is not None:
                ...
                self.shapeMode = (
                    FULLSHAPE_MODE if cfg.defaultFullSpace else HALFSHAPE_MODE
                )
    ...
    def onActivate(self):
        ...
        threading.Thread(target=self.updateUI, daemon=True).start()
    ...
    def filterKeyDown(self, keyEvent):
        ...
        # if keyCode:  # 確保 keyCode 是非零的有效值
        #     if keyEvent.isKeyDown(keyCode):
        #         charCode = keyEvent.charCode
        #         charStr = chr(charCode) if charCode > 0 else ""
        #         logger.info("filterKeyDown: charCode=%d, keyCode=%d, charStr=%s" % (charCode, keyCode, charStr))
        ...
        if keyEvent.isKeyDown(VK_CONTROL):
            ...
            if (
                cfg.easySymbolsWithCtrl
                and keyEvent.isPrintableChar()
                and self.langMode == CHINESE_MODE
            ):
                return True
    ...
    def updateUI(self):
        while True:
            with open(r"C:\Users\super\Desktop\PolyKey\pimeoutput.txt", "r", encoding="utf-8") as ff:
                outputfile = ff.readlines()
                if self.dyna_best_result != outputfile[0].strip():
                    self.setCompositionString(outputfile[0].strip())
                self.dyna_best_result = outputfile[0].strip()
    ...
    def onKeyDown(self, keyEvent):
        ...
        #LOG
        logger.info("charStr: %s", charStr)
        # logger.info("current time: %s", time.time())
        # logger.info("lastKeyDownTime: %s", self.lastKeyDownTime)
        # logger.info("keepTypingTime: %s", self.keepTypingTime)
        # logger.info("InputString= %s", self.temp_inputstring)
        
        
        #INPUT
        with open(r"C:\Users\super\Desktop\PolyKey\pimeinput.txt", "w", encoding="utf-8") as f:
        ...
        # ENTER FROM COMP to COMMIT 
        if keyCode == VK_RETURN and self.dyna_best_result != "" and self.showCandidates == False:
            logger.info("here is when enter key pressed when candidate list is not shown")
            self.setCommitString(self.dyna_best_result)
            self.temp_inputstring = ""
            keyHandled = True
            self.check_commit = True
            # logger.info("Enter key pressed")
            # f=open(r"C:\Users\super\Desktop\PolyKey\pimeinput.txt", "r+")
            # f.seek(0)
            # f.truncate()
        
        if keyCode == VK_RETURN and self.showCandidates == True:
            logger.info("here is when enter key pressed when candidate list is shown")
            # self.setCompositionString(self.dyna_best_result)
            self.showCandidates = False
            self.setShowCandidates(False)
            # logger.info("Enter key pressed for candidate list")
            # logger.info("selKey: %s", selKey)
            # After_choose_str = self.dyna_best_result.replace(self.dyna_best_result[int(self.cursor_position)-1],self.candidates[self.int_selkey-1])
            # self.setCompositionString(After_choose_str)
        ...
        if self.check_commit == False:
            self.setCompositionString(self.dyna_best_result) # is there a conflict between the composition and the commit string?
        else:
            self.check_commit = False
            
            
        #counting down for 0.5 sec, not related to this function
        # if (time.time() - self.keepTypingTime) < 0.5:
        #     self.setCompositionString(self.temp_inputstring)
        #     self.setCompositionCursor(len(self.temp_inputstring))
        # else:
        #     self.setCompositionString(self.dyna_best_result)
        #     self.setCompositionCursor(len(self.dyna_best_result))
        
        # ENTER CANDIDATELIST
        # selKey = cfg.getSelKeys()[self.candidateCursor]
        # logger.info("selKey: %s", selKey)
        # self.int_selkey = int(selKey)
        
        # LEFT
        # if keyCode == VK_LEFT and self.showCandidates == False:
        #     with open(r"C:\Users\super\Desktop\PolyKey\pimeinput.txt", "w", encoding="utf-8") as f:
        #         f.write(self.temp_inputstring+"\nl")
            
        #     with open(r"C:\Users\super\Desktop\PolyKey\pimeoutput.txt", "r", encoding="utf-8") as ff:
        #         outputfile = ff.readlines()
        #         self.dyna_best_result = outputfile[0].strip()
        #         if len(outputfile) > 1:
        #             self.cursor_position = outputfile[1]
        #             self.setCompositionCursor(int(self.cursor_position))
        #         self.setCompositionString(self.dyna_best_result)
        # RIGHT
        # if keyCode == VK_RIGHT and self.showCandidates == False:
        #     with open(r"C:\Users\super\Desktop\PolyKey\pimeinput.txt", "w", encoding="utf-8") as f:
        #         f.write(self.temp_inputstring+"\nr")
        ...
        if keyEvent.isPrintableChar():
            ...
            if self.langMode == ENGLISH_MODE:  # 英文模式
                ...
                # Ctrl + 數字(0-9)
                    ...
                    keyEvent.isKeyToggled(VK_NUMLOCK)
                    and keyCode >= VK_NUMPAD0
                    and keyCode <= VK_DIVIDE
                ):
            ...
            if self.showCandidates:
                candCursor = self.candidateCursor  # 目前的游標位置
                # candCount = len(self.candidateList)  # 目前選字清單項目數
                candCount = self.candidatelength

                if (
                    keyEvent.isKeyDown(VK_CONTROL) and keyCode == VK_DELETE
                ):  # 處理刪除詞彙
                    ...
                    try:
                        ...
                        cursor.execute(
                            "SELECT * FROM userphrase_v1 WHERE phrase=:delete_phrase",
                        ...
                        if result is None:
                            phraseConnect.close()
                            self.showMessage(
                                "詞彙「" + delete_phrase + "」不存在，無法刪除", 2
                            )
                        else:
                            cursor.execute(
                                "DELETE FROM userphrase_v1 WHERE phrase=:delete_phrase",
                                {"delete_phrase": delete_phrase},
                            )
                ...
                if keyCode == VK_RETURN:  # 按下 Enter 鍵
                    ...
                    keyHandled = True #orginal is True
                # 更新選字視窗游標位置
                self.setCandidateCursor(candCursor)

            if not keyHandled:  # 按鍵還沒被處理過
                logger.info("C1 keyHandled : False")
        ...
        if keyHandled and chewingContext.keystroke_CheckIgnore():
            # logger.info("C2 keyHandled : True")
            ignoreKey = True
            
        if (not ignoreKey) and keyCode != VK_RETURN:  # 如果這個按鍵是有意義的，新酷音有做處理 (不可忽略)
            # 處理選字清單
            self.candidates=[]
            logger.info("Open the candidate list")
            with open(r"C:\Users\super\Desktop\PolyKey\pimeoutput.txt", "r",encoding="utf-8") as ff:
                temp_output = ff.readlines()
                if len(temp_output) > 1:
                    candidate_from_output = temp_output[1].strip().split(",")
                    for i in candidate_from_output:
                        self.candidates.append(i)
                self.candidatelength = len(self.candidates)
                
            # if chewingContext.cand_TotalChoice() > 0:  # 若有候選字/詞
            if self.candidatelength > 0 and keyCode == VK_DOWN:
                # 要求新酷音引擎列出每個候選字
                # self.candidates = []
                ...
                # 檢查選字清單是否改變 (沒效率但是簡單)
                if self.candidates != self.candidateList:
                    self.setCandidateList(self.candidates)  # 更新候選字清單
                    self.setShowCandidates(True)
                    if (
                        cfg.leftRightAction == 0 or cfg.upDownAction == 0
                    ):  # 如果啟用選字清單內使用游標選字
                        if (
                            keyCode == VK_LEFT
                        ):  # 如果按下左鍵，設定游標為選字清單前一頁最後一個
            ...
            if chewingContext.commit_Check():
                ...
                # logger.info("Commit %s", commitStr.encode("utf-8")) #Test 10/26
                logger.info("NoewselKey: %d", self.int_selkey)
                self.candidateword = self.candidates[self.int_selkey-1]
                commitStr = self.candidateword
                # 如果使用打繁出簡，就轉成簡體中文
                if self.outputSimpChinese:
                    commitStr = self.opencc.convert(commitStr)
                    
                # After_choose_str = self.dyna_best_result.replace(self.dyna_best_result[self.cursor_position-1],commitStr)
                # self.setCommitString(After_choose_str)  # 設定要輸出的 commit string
                # self.setCommitString(commitStr)  # 設定要輸出的 commit string
                # self.setCommitString(self.dyna_best_result)  # 設定要輸出的 commit string
            ...
            if chewingContext.bopomofo_Check():
                # 當其他輸入法下的情況(不完整的bopomofo)，還是可以選字
                # if self.candidates != self.candidateList:
                #     self.setCandidateList(self.candidates)
                #     self.setShowCandidates(True)
                bopomofoStr = ""
    ...
    def filterKeyUp(self, keyEvent):
        ...
        if (
            self.lastKeyEvent
            and self.lastKeyEvent.keyCode == VK_SHIFT
            and keyEvent.keyCode == VK_SHIFT
        ):
            ...
            if chewingConfig.switchLangWithShift:
                ...
                if (
                    chewingConfig.switchLangWithWhichShift
                    == SWITCH_LANG_WITH_BOTH_SHIFT
                ):
                    pass
                elif (
                    chewingConfig.switchLangWithWhichShift
                    == SWITCH_LANG_WITH_LEFT_SHIFT
                    and not self.isPressed(VK_LSHIFT)
                ):
                    # logger.info("check1 : %s", self.lastKeyDownTime)
                    self.lastKeyDownTime = 0.0
                    return False
                elif (
                    chewingConfig.switchLangWithWhichShift
                    == SWITCH_LANG_WITH_RIGHT_SHIFT
                    and not self.isPressed(VK_RSHIFT)
                ):
                    # logger.info("check2 : %s", self.lastKeyDownTime)
                    self.lastKeyDownTime = 0.0
                    return False
        ...
        if chewingConfig.enableSwitchTCSC and self.lastKeyEvent:
            if (
                ...
                and self.lastKeyEvent.keyCode == VK_F12
                and keyEvent.keyCode == VK_F12
            ):
        ...
        # logger.info("check3 : %s", self.lastKeyDownTime)
        if self.lastKeyDownTime != 0.0:
            self.keepTypingTime = self.lastKeyDownTime
        self.lastKeyDownTime = 0.0
        return False
    ...
    def onKeyUp(self, keyEvent):
        ...
        if (
            ...
            and not self.chewingContext.bopomofo_Check()
        ):
            if self.lastKeyEvent:
                if self.lastKeyEvent.isKeyDown(
                    VK_RSHIFT
                ) and self.compositionCursor < len(self.compositionString):
                    self.chewingContext.handle_Right()
                    self.setCompositionCursor(self.chewingContext.cursor_Current())
                if (
                    self.lastKeyEvent.isKeyDown(VK_LSHIFT)
                    and self.compositionCursor > 0
                ):
                    self.chewingContext.handle_Left()
                    self.setCompositionCursor(self.chewingContext.cursor_Current())

            # logger.info("check4 : %s", self.lastKeyDownTime)
            if self.lastKeyDownTime != 0.0:
                self.keepTypingTime = self.lastKeyDownTime
            self.lastKeyDownTime = 0.0
            return True
    ...
    def onCommand(self, commandId, commandType):
        ...
        if (
            commandId == ID_SWITCH_LANG and commandType == COMMAND_LEFT_CLICK
        ):  # 切換中英文模式
            self.toggleLanguageMode()
        elif (
            commandId == ID_SWITCH_SHAPE and commandType == COMMAND_LEFT_CLICK
        ):  # 切換全形/半形
            self.toggleShapeMode()
        elif (
            commandId == ID_SETTINGS or commandId == ID_USER_PHRASE_EDITOR
        ):  # 開啟設定工具 or 編輯辭庫
            if commandId == ID_USER_PHRASE_EDITOR:  # 編輯使用者辭庫
                tool_name = "user_phrase_editor"
            else:  # 輸入法設定
                tool_name = "config_tool"
            config_tool = '"{0}" {1}'.format(
                os.path.join(self.curdir, "config_tool.py"), tool_name
            )
